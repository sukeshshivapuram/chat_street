...
class InterCompanyTransferEpt(models.Model):
    ...
    def onchange_source_warehouse_id(self):
        ...
        if self.env.user.sto_warehouse_line.from_warehouse_id:
            if self.source_warehouse_id:
                if self.env.user.sto_warehouse_line.from_warehouse_id.id != self.source_warehouse_id.id:
                    raise UserError(_('You are not a authorised peron for The %s warehouse.')%self.source_warehouse_id.name)
    ...
    def onchange_destination_warehouse_id(self):
        ...
        if self.env.user.sto_warehouse_line.from_warehouse_id:
            if self.destination_warehouse_id:
                if self.destination_warehouse_id.id not in  self.env.user.sto_warehouse_line.to_warehouse_ids.ids :
                    raise UserError(_('You are not a authorised peron for The %s warehouse.')%self.destination_warehouse_id.name)
    ...
    def create(self, vals):
        ...
        #         'odoov13_8.ir_sequence_inter_company_transaction').ids
        # elif vals.get('type', '') == 'ict_reverse':
        #     sequence_id = self.env.ref(
        #         'odoov13_8.ir_sequence_reverse_inter_company_transaction').ids
        # elif vals.get('type', '') == 'internal':
        #     sequence_id = self.env.ref(
        #         'odoov13_8.ir_sequence_internal_transfer').ids
        # if sequence_id:
        #     record_name = self.env['ir.sequence'].browse(sequence_id).next_by_id()
        record_name = self.env['ir.sequence'].next_by_code('inter.company.transfer.ept')
    ...
    # def create(self, vals):
    #     vals['name'] = 
    #     return super(InterCompanyTransferEpt, self).create(vals)
        ...
        for record in self:
            if record.with_context(context).check_user_validation():
                ...
                configuration_record = record.env.ref(
                    'odoov13_8.inter_company_transfer_config_record')
    ...
    def create_internal_transfer(self):
        ...
        for line in self.inter_company_transfer_line_ids:
            print("line.product_id.uom_idline.product_id.uom_id",line.product_id.uom_id.category_id.name)
            # print("line.product_id.uom_idline.product_id.uom_id",line.product_id.uom_id.category_id.name)
            # pdb.set_trace()
    ...
    def reverse_invoices(self):
        ...
        configuration_record = self.env.ref('odoov13_8.'

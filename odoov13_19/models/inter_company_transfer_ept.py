...
class InterCompanyTransferEpt(models.Model):
    ...
    def create(self, vals):
        ...
        if vals.get('type', '') in ['ict', '']:
            sequence_id = self.env.ref(
                'odoov13_19.ir_sequence_inter_company_transaction').ids
        elif vals.get('type', '') == 'ict_reverse':
            sequence_id = self.env.ref(
                'odoov13_19.ir_sequence_reverse_inter_company_transaction').ids
        elif vals.get('type', '') == 'internal':
            sequence_id = self.env.ref(
                'odoov13_19.ir_sequence_internal_transfer').ids
        if sequence_id:
        ...
        for record in self:
            if record.with_context(context).check_user_validation():
                ...
                configuration_record = record.env.ref(
                    'odoov13_19.inter_company_transfer_config_record')
    ...
    def reverse_invoices(self):
        ...
        configuration_record = self.env.ref('stock_transfer_order_curefit.'
